...
dnl
dnl @version $Id: acx-pthread.m4,v 1.8 2003/06/26 20:40:26 richi Exp $
...
if test x"$acx_pthread_ok" = xyes; then
        ...
        if  test x"$acx_pthread_sigmask_ok" = xyes; then
                AC_MSG_RESULT([yes])
                AC_DEFINE(HAVE_PTHREAD_SIGMASK,, [have pthread_sigmask function])
        else
                AC_MSG_RESULT(no)
        fi
        CFLAGS="$save_CFLAGS"
