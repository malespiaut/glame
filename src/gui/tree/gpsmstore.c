...
GType
glame_gpsm_store_get_type (void)
{
  ...
  if (!gpsm_store_type)
	...
	NULL
      ...
      gpsm_store_type = g_type_register_static (G_TYPE_OBJECT, "GlameGpsmStore",
						&gpsm_store_info, 0);
...
static void
glame_gpsm_store_class_init (GlameGpsmStoreClass *class)
{
...
static void
glame_gpsm_store_finalize (GObject *object)
{
  (* parent_class->finalize)(object);
}

static void
glame_gpsm_store_tree_model_init (GtkTreeModelIface *iface)
{
	/* FIXME! */
...
static void
glame_gpsm_store_init (GlameGpsmStore *tree_store)
{
...
{
	GlameGpsmStore *store = GLAME_GPSM_STORE(glsig_handler_private(handler));
	GtkTreePath *path;
	GtkTreeIter iter;
	gpsm_item_t *item;
	...
	iter.user_data = item;
	path = gtk_tree_store_get_path(store, &iter);
	gtk_tree_model_row_changed(store, path, &iter);
	gtk_tree_path_free(path);
}

static void handle_item_addremove_1(GlameGpsmStore *store, gpsm_item_t *item, int sig);
static void handle_item_addremove(glsig_handler_t *handler, long sig, va_list va);
static void handle_item_addremove_r(GlameGpsmStore *store, gpsm_item_t *grp, int sig)
{
	gpsm_item_t *item;
...
GlameGpsmStore *
glame_gpsm_store_new (gpsm_item_t *root)
{
			  ...
			  GtkTreeIter   *iter)
{
  ...
  iter->user_data = gpsm_grp_next(gpsm_item_parent(iter->user_data), iter->user_data);
  if (iter->user_data)
    return TRUE;
  else
    return FALSE;
			      ...
			      GtkTreeIter  *parent)
{
  ...
  g_return_val_if_fail (parent == NULL
			|| (parent->user_data != NULL
			    && GPSM_ITEM_IS_GRP(parent->user_data)), FALSE);

  if (parent)
    iter->user_data = gpsm_grp_first(parent->user_data);
  else
    iter->user_data = gpsm_grp_first(GLAME_GPSM_STORE(tree_model)->root);
