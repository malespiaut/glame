 ...
 * list.h
 *
 * $Id: list.h,v 1.20 2001/06/06 12:02:50 richi Exp $
 * 
 ...
 * for internal use. */
#define list_safe_foreach(listptr, type, member, dummy, entryvar) for (entryvar = list_entry((listptr)->next, type, member), (struct list_head *)dummy = (listptr)->next->next; (&entryvar->member != (listptr) && (!list_empty(&entryvar->member) || DERROR_eval("list_safe_foreach remove error"))) || (entryvar = NULL); entryvar = list_entry((struct list_head *)dummy, type, member), (struct list_head *)dummy = ((struct list_head *)dummy)->next) 
