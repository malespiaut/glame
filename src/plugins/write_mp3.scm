(let ((newplugin (glame_plugin_define (let* ((net (filter-new))
	(Resample-1 (filter-add-node net (filter-new (plugin-get "Resample")) "Resample-1"))
	(Resample (filter-add-node net (filter-new (plugin-get "Resample")) "Resample"))
	(render (filter-add-node net (filter-new (plugin-get "render")) "render"))
	(pipe_out (filter-add-node net (filter-new (plugin-get "pipe_out")) "pipe_out")))
   ((lambda (pdb)
