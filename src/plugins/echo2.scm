(let ((newplugin (glame_plugin_define (let* ((net (filter-new))
	(extend (filter-add-node net (filter-new (plugin-get "extend")) "extend"))
	(mix (filter-add-node net (filter-new (plugin-get "mix")) "mix"))
	(one2n (filter-add-node net (filter-new (plugin-get "one2n")) "one2n"))
	(delay (filter-add-node net (filter-new (plugin-get "delay")) "delay"))
	(volume-adjust (filter-add-node net (filter-new (plugin-get "volume_adjust")) "volume-adjust")))
