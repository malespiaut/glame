...
/* The $(LADSPA_PATH) value to use when the environment variable has
   not been set. */
...
/* Probably should put these in glplugin.h, however I've a bad feeling
   about all this - should I really be allowed to use these calls (had
   to remove `static' identifiers in glplugin.c). I'll leave this to
   the designers. FIXME */
plugin_t *alloc_plugin(const char *name);
void free_plugin(plugin_t *p);
...
#define n psNode
  ...
  psDescriptor = (const LADSPA_Descriptor *)(psNode->filter->private);
  ...
  for (lPortIndex = 0; lPortIndex < lPortCount; lPortIndex++) {
	...
	iNTo1_NR++;
	psPipe = filternode_get_input(psNode,
	...
	psPipe = filternode_get_output(psNode,
				       psDescriptor->PortNames[lPortIndex]);
	if (!psPipe)
	  FILTER_ERROR_RETURN("LADSPA plugins require all "
  ...
  if (lSampleRate == 0) {
    ...
    psParam = filternode_get_param(psNode, "GLAME Sample Rate");
    if (psParam)
  ...
  for (lPortIndex = 0; lPortIndex < lPortCount; lPortIndex++) {
	...
	psParam = filternode_get_param(psNode,
				       psDescriptor->PortNames[lPortIndex]);
	if (!psParam)
	  FILTER_ERROR_RETURN("LADSPA plugins require all "
			      "parameters be set");
	...
	psNTo1_State[iNTo1_Index++].in
	  ...
	  = filternode_get_input(psNode,
				 psDescriptor->PortNames[lPortIndex]);
      }
      else /* i.e. LADSPA_IS_PORT_OUTPUT(iPortDescriptor) */
	ppsAudioPorts[lPortIndex]
	  = filternode_get_output(psNode,
				  psDescriptor->PortNames[lPortIndex]);
    }
  ...
  while (nto1_tail(psNTo1_State, iNTo1_NR) == 0) {
	...
	ppsBuffers[lPortIndex]
	  = sbuf_make_private(sbuf_alloc(lRunSampleCount, psNode));
	psDescriptor->connect_port(psLADSPAPluginInstance,
				   lPortIndex,
...
/* This call examines a LADSPA plugin descriptor and uses it to set up
   a of GLAME plugin. */
int
installLADSPAPlugin(const LADSPA_Descriptor * psDescriptor,
		    plugin_t * psPlugin) {
...
static void
processLADSPALibrary(const char * pcFilename,
		     LADSPA_Descriptor_Function fDescriptorFunction) {

  const LADSPA_Descriptor * psDescriptor;
  plugin_t * psSubsidPlugin;
  unsigned long lPluginIndex;

  for (lPluginIndex = 0;
       (psDescriptor = fDescriptorFunction(lPluginIndex)) != NULL;
       lPluginIndex++) {

    /* Use the LADSPA label to describe the plugin. Other alternatives
       include the LADSPA name. Note that labels are NOT guaranteed
       unique, so potential clashes happen when two libraries use the
       same label within them. This can be handled by modifying the
       label (e.g. in the Windows style "lpf" and "lpf(2)" or by
       concatenating the plugin library name "cmt/lpf" etc). FIXME:
       current behaviour is that in the event of a label clash, only
       one version survives (see add_plugin below). */
    psSubsidPlugin = alloc_plugin(psDescriptor->Label);
    if (!psSubsidPlugin)
      return;

    /* Give each individual plugin a dlopen() handle to the
       library. This protects the library from being dropped from
       memory while it is still needed by a plugin. */
    psSubsidPlugin->handle = dlopen(pcFilename, RTLD_NOW);
    if (!psSubsidPlugin->handle) {
      free_plugin(psSubsidPlugin);
      return;
    }

    /* Now we perform the translation of the LADSPA descriptor
       information into GLAME information. */
    if (installLADSPAPlugin(psDescriptor, psSubsidPlugin) == -1) {
      free_plugin(psSubsidPlugin);
      /* This may be non fatal (e.g. descriptor is incompatible with
         GLAME for some reason). Don't give up on the rest of the
         plugins in the library. */
      continue;
    }

    /* And finally we fire off the plugin into the hashtable. */
    if (add_plugin(psSubsidPlugin) == -1) {
      free_plugin(psSubsidPlugin);
      /* This may be non fatal (i.e. the label is already in the hash
         table.) */
      continue;
    }
  }
...
LADSPADirectoryPluginSearch(const char * pcDirectory) {
  ...
  while (1) {
    ...
    pvPluginHandle = dlopen(pcFilename, RTLD_NOW);
	...
	processLADSPALibrary(pvPluginHandle,
			     fDescriptorFunction);
      }
      else {
...
int
ladspa_register(plugin_t * psPlugin) {

  /* FIXME: This function doesn't actually use its psPlugin
     parameter. Should it? If so, how? */

  char * pcBuffer;
  const char acDefaultLADSPAPath[] = DEFAULT_LADSPA_PATH;
  const char * pcEnd;
  const char * pcLADSPAPath;
  const char * pcStart;

  /* Reach item from the LADSPA_PATH environment variable (or the
     default path if this isn't defined.) */

  pcLADSPAPath = getenv("LADSPA_PATH");
  if (!pcLADSPAPath)
    pcLADSPAPath = acDefaultLADSPAPath;
  ...
  while (*pcStart != '\0') {
    ...
    LADSPADirectoryPluginSearch(pcBuffer);
